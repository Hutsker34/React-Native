...
const Snowflake = ({size, speed}) => {
  ...
  const animatedStyles = useAnimatedStyle(() => {
    return {
      transform: [{ translateX: x.value }, { translateY: y.value + speed}],
      position: 'absolute',
  ...
  useEffect(() => {
    ...
    y.value = withRepeat(withTiming(maxh + 50, { duration: 10000 }), -1, false); // Adjust timing for speed
